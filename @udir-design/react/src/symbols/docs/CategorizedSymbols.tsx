...
type CategoryWithSymbols = {
  category: string;
  symbols: UdirSymbol[];
};

export const CategorizedSymbols = ({
  symbols,
  selectedSymbol,
  setSelectedSymbol,
}: {
  symbols: CategoryWithSymbols[];
  selectedSymbol: UdirSymbol | null;
  setSelectedSymbol: Dispatch<SetStateAction<UdirSymbol | null>>;
}) => {
  return (
    <div className={styles.symbolSection}>
      {symbols.length === 0 && <NoResults />}
      {symbols.map((section) => {
        return (
          <div key={section.category}>
            {section.category !== 'Symboler' && (
            ...
            <div className={styles.symbolGrid}>
              {section.symbols.map((symbol) => {
                const Symbol =
                  Symbols[`${symbol.id}Symbol` as keyof typeof Symbols];
                if (Symbol === undefined) {
                  return null;
                }
                return (
                  <Button
                    key={symbol.id + section.category}
                    variant="tertiary"
                    data-size="lg"
                    onClick={() => {
