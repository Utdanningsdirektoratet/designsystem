...
Tilstanden til `Select` kan kontrolleres ved å bruke value og onChange. Dette er nyttig når du vil at `Select` skal oppdatere seg selv basert på endringer i andre komponenter eller tilstander.
...
`Select` støtter `aria-readonly`-attributtet for å gjøre feltet skrivebeskyttet, og gir en visuell indikasjon
som skiller seg fra redigerbare felter. Vi unngår likevel `aria-readonly` så langt det lar seg gjøre, fordi
