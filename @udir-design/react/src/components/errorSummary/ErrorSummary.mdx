...
import { ErrorSummary } from '../alpha';
...
`ErrorSummary` er påkrevd for å få riktig aria attributter (`aria-labelledby`, `role='alert'`, osv.).
...
I noen tilfeller kan det likevel være bedre å vise oppsummeringen i toppen, det kan du lese mer om i [artikkelen om feilmeldinger hos Digdir](https://designsystemet.no/monstre/feilmeldinger#oppsummering-av-flere-feilmeldinger).
