...
mangler som må rettes på en side for å komme videre.
...
- vise feilmeldinger for enkeltfelt, bruk heller `ValidationMessage` i [`TextField`](/docs/components-textfield--docs), [`Select`](/docs/components-select--docs), [`Checkbox`](/docs/components-checkbox--docs) osv.
