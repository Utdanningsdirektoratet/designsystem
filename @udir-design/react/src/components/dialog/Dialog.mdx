...
Her bruker vi `useRef` for å få tilgang til dialogen, og kaller `.showModal()` når knappen trykkes. Merk at knappen som åpner dialogen da bør ha `aria-haspopup="dialog"`-attributtet.

<Canvas of={DialogStories.WithoutDialogTriggerContext} />

Du kan også bruke `command="show-modal"` eller `command="--show-non-modal"` og `commandfor="DIALOG-ID"` for å åpne dialogen fra en ekstern trigger.

### Lukk ved klikk utenfor

Vi bruker `closedby="any"` proppen for å lukke dialogen når brukeren klikker utenfor. Vi legger til polyfill så `closedby="any"` også fungerer i eldre Safari.
