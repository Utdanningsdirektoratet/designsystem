...
- velge mellom flere enn 7 svaralternativer, bruk heller [`MultiSuggestion`](/docs/components-multisuggestion--docs)
...
`Checkbox` skal alltid ha et tilgjengelig navn. I de fleste tilfeller er dette `label`. I spesielle tilfeller kan det gi mening å unnlate `label`, og heller bruke `aria-label` eller `aria-labelledby`. Et eksempel på begge disse aria-attributtene er [bruken av `Checkbox` i tabell](#velge-alle-alternativer-med-en-overordnet-avmerkingsoks) lenger opp.
