...
Både `Logo` og `LogoSymbol` har en pride-variant som kan aktiveres ved å sette `variant`-propen til `pride`. Varianten brukes i juni for å markere Pride. Logoen har innebygd støtte for fargemodusene `light`, `dark` og `auto`.

<Canvas of={LogoStories.PrideVariants} />

<Canvas of={LogoSymbolStories.PrideVariants} />
