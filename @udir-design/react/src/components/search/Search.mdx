...
Tilstanden til `Search.Input` kan kontrolleres ved å bruke `value` og `onChange`. Dette er nyttig når du vil at `Search` skal oppdateres basert på endringer i andre komponenter eller tilstander.
...
`Search.Button` og `Search.Clear` har innebygget støtte for bokmål, nynorsk og engelsk. [Slik støtter du flere språk](/docs/components-språkstøtte--docs).

Send innhold til `Search.Button` eller `aria-label` til `Search.Clear` dersom du vil angi tekstene eksplisitt.

Følgende tekstvariabler er tilgjengelige for overstyring:

<CssLanguageVariables
  variables={['--udsc-search-button-text', '--udsc-search-clear-label']}
/>
