import { IncludeMarkdown } from '.storybook/docs/components';
import { HideToc } from '.storybook/docs/components';

<HideToc />

# Slik støtter du flere språk

Udirs designsystem støtter flerspråklige løsninger. I de fleste komponentene håndterer du språkvalg selv ved å sende inn tekst som props eller children.

Noen komponenter har tekst som er forhåndsdefinert. Disse komponentene vil bruke riktig språk dersom [`lang`-attributtet](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/lang) i løsningen er satt til et språk vi har oversettelser for.

Det er bare de forhåndsdefinerte tekstene som styres av `lang` og CSS-variablene. Tekst du sender inn gjennom props eller `children`, må du oversette selv. En eksplisitt verdi i en prop overstyrer alltid komponentens standardtekst.

Det vil fungere å sette `lang` på hvilken som helst forelder, men gjerne sett det på `<html>` for å sikre konsistens.

```jsx
<div lang="en">
  <FileUpload />
</div>
```

Komponentene med forhåndsdefinert tekst støtter språkene `"nb"` (bokmål), `"nn"` (nynorsk) eller `"en"` (engelsk). Ønsker du å støtte andre språk i disse komponentene, kan du sette standardtekstene gjennom CSS-variabler. Det gjør du ved å overstyre variablene i `lang`-attributtet for ønsket språk:

```css
/* Oversettelser for nordsamisk */
[lang|='se' i] {
  --udsc-fileUpload-uploading-text: 'Viežžamin…';
  --udsc-fileUpload-chooseFile-text: 'Lasit fiilla';
  /* ...flere tekstvariabler... */
}
```

Vi bruker `[lang|='se' i]` i stedet for `[lang='se']` slik at oversettelsen også gjelder når `lang` inneholder en region, for eksempel `se-NO`.

Se hvilke tekstvariabler som er tilgjengelige for overstyring i dokumentasjonen for komponentene:

- [Breadcrumbs](/docs/components-breadcrumbs--docs#språkstøtte)
- [DemoBanner](/docs/components-demobanner--docs#språkstøtte)
- [Dialog](/docs/components-dialog--docs#språkstøtte)
- [Field](/docs/components-field--docs#språkstøtte)
- [FieldNecessity](/docs/components-fieldnecessity--docs#språkstøtte)
- [FileUpload](/docs/components-fileupload--docs#språkstøtte)
- [FormSummary](/docs/components-formsummary--docs#språkstøtte)
- [Pagination](/docs/components-pagination--docs#språkstøtte)
- [Search](/docs/components-search--docs#språkstøtte)
- [Suggestion](/docs/components-suggestion--docs#språkstøtte)
- [TableOfContents](/docs/components-tableofcontents--docs#språkstøtte)
- [useDrilldownTable](/docs/hooks-usedrilldowntable--docs#språkstøtte)
