...
### Kontrollere Switch utenfra

Du kan kontrollere `Switch` utenfra ved å bruke `checked` og `onChange`.
...
Vi bruker `role="switch"` for å fortelle skjermlesere at dette er en `Switch`. Dette gjør at skjermlesere kan gi brukeren informasjonen de trenger og at den kan skrus av og på.
Det er ikke lagt på `aria-checked` siden dette er ikke nødvendig når `Switch` er en input med `type="checkbox"`.
