...
`Suggestion.Toggle` legger til en knapp som åpner og lukker listen med alternativer. Angi en annen `aria-label` dersom standardteksten ikke beskriver innholdet i listen.
...
Du kan angi tekstene eksplisitt med `aria-label` på `Suggestion.Clear` og `Suggestion.Toggle`, og med `singular` og `plural` på `Suggestion.List`.
