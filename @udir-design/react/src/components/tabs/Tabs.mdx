...
`Tabs` har også en "card" variant som minner om faner i et filsystem. Denne varianten passer til å visuelt knytte fanene til innholdet i panelene.

<Canvas of={TabsStories.CardVariant} />

Dersom du ønsker å ramme inn innholdet i panelene setter du `variant="card"` både på `Tabs` og på `Tabs.Panel`.

<Canvas of={TabsStories.CardPanel} />
