...
Tabelloverskrifter kan settes til tinted farge. Dette kontrolleres med to props for column- og row headers respektivt: `tintedColumnHeader` og `tintedRowHeader`. For å sette farge på radoverskrifter må `scope='row'` eller `scope='rowGroup'` brukes på `Table.HeaderCell`. Vær bevisst på når du velger å bruke tinted headere, og vær konsekvent i hvert system.
...
- **Overskrifter:**
  For at brukerne skal kunne navigere i innholdet med skjermleser, er det viktig å bruke rad- og/eller kolonneoverskrifter (`<Table.HeaderCell>`). Bruk `scope="row"` og `scope="col"` for å angi om overskriften gjelder en rad eller kolonne der det er nødvendig. Det er nødvendig i alle typer tabeller som har mer enn én rad eller kolonne med overskrifter. Hvis det er behov for å spare plass, kan vi visuelt skjule én eller flere overskrifter.
- **Caption-element:**
