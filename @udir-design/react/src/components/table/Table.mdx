...
`Table.HeaderCell` har en `sortable` prop som kan brukes for å vise at raden er sorterbar.
...
- **Overskrifter:**
  Alle tabeller må ha overskrifter for hver kolonne (`<Table.HeaderCell>`). Unntaket er kolonner som kun inneholder interaktive elementer, de skal ha ledetekst. Se eksempel ["Med skjemaelementer"](#med-skjemaelementer). Hvis det er behov for å spare plass, kan vi visuelt skjule én eller flere overskrifter.
- **Kolonneoverskrifter:**
  For at brukerne skal kunne navigere i innholdet med skjermleser, må det være overskrifter i kolonnene. Hvis det er behov for å spare plass, kan vi visuelt skjule én eller flere overskrifter.
  Bruk `scope="row"` og `scope="col"` for å angi om overskriften gjelder en rad eller kolonne.
- **Caption-element:**
...
- **Tilpasning for mobil:**
  ...
  De vanligste tabellene som brukes på mobil er en tabell som ruller horisontalt eller omgjøring til listevisning.
