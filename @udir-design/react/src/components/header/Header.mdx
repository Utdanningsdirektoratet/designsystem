...
På mindre skjermer flytter du enten `Header.UserButton` inn i `Header.Menu`, eller bytter ut `Header.UserButton` med en [`Avatar`](/docs/components-avatar--docs) som knapp.
...
<Unstyled>
  ...
  >
    ...
    <Table.Body>
      ...
      <Table.Row>
        ...
        <Table.Cell>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            ...
            <li>Brukeren trenger handlinger</li>
          </ul>
        </Table.Cell>
      </Table.Row>
      <Table.Row>
      ...
      <Table.Row>
        <Table.HeaderCell scope="row">Lenker + meny</Table.HeaderCell>
        <Table.Cell>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            <li>2–4 hovedsider</li>
            <li>Brukeren bytter ofte mellom dem</li>
...
`Header` og alle underkomponenter tilpasser seg automatisk til mindre skjermer i eksemplene. På skjermer under 480px får komponentene `data-size="sm"` for kompakt visning. Dette styres av en størrelsesmodus som kan settes på hele systemet. [Les mer om hvordan du setter størrelsesmoduser hos Digdir](https://www.designsystemet.no/no/fundamentals/code/sizes).
