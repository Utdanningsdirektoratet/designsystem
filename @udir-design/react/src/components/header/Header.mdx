...
Bruk [`<nav>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/nav) rundt navigasjonsområder.

- Knytt området til en overskrift med `aria-labelledby` og en `id` på overskriften.
- Hvis navigasjonsområdet ikke har en overskrift, bruk `aria-label` i stedet.
- Se meny-eksemplene over for hvordan dette kan gjøres i praksis.
