...
`Tooltip` viser kort informasjon når brukeren holder musepekeren over (`hover`) eller fokuserer på et element (`active`). Den brukes til sekundær informasjon, for eksempel til å forklare hva et symbol betyr.
...
`Tooltip` rendrer en `span`, så du kan bruke den i f.eks. `div`, men ikkje inni `svg`.
