...
Navnet på gruppen settes i `title`-propen. I lengre skjemaer kan du spare plass ved å lukke enkelte grupper. Dette styres med `open`-propen. Du kan også bruke `activeStepLabel`-propen for å vise hvilket steg i gruppen som er aktivt når gruppen er lukket. Dette håndteres automatisk ved bruk av [`useFormNavigation`](/docs/utilities-useformnavigation--docs) dersom du setter `label` i `getStepProps` på `FormNavigation.Step`.
