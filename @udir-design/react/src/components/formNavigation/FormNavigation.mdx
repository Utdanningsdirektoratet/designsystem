...
`FormNavigation` hjelper brukeren med å navigere mellom seksjoner og steg i et skjema. Komponenten gir oversikt over fremdriften og lar brukeren enkelt hoppe mellom ulike deler av skjemaet. Du bygger opp navigasjonen ved hjelp av subkomponenter, [`useFormNavigation`](/docs/utilities-useformnavigation--docs) og [hjelpefunksjoner for skjema](/docs/utilities-hjelpefunksjoner-for-skjema--docs) for å håndtere tilstander og interaksjoner.

**Passer til å**

- navigere i flertrinnsskjemaer
- gi brukeren oversikt over fremdriften i et skjema
...
`FormNavigation.Step` representerer et steg i skjemaet. Hvert steg inneholder informasjon eller spørsmål knyttet til ett tema. Et steg kan også brukes som informasjonsside, oppsummeringsside, innsendingsside eller kvitteringsside.

Tittelen legges inn som `children`. Bruk `variant`-propen for å vise riktig ikon automatisk for stegetypen (`variant='info' | 'summary' | 'submission' | 'confirmation' | 'default'`).

<Canvas of={FormNavigationStories.SingleStep} />

### Gruppering av steg

`FormNavigation.Group` er en samling steg som representerer relaterte sider i skjemaet. Bruk `FormNavigation.Step` som `children`. Grupper brukes for å organisere skjemaet i logiske seksjoner, slik at det blir enklere for brukeren å forstå strukturen.

Navnet på gruppen settes i `title`-propen. I lengre skjemaer kan du spare plass ved å lukke enkelte grupper med `open`-propen. Det aktive steget i gruppen vises også når gruppen er lukket.

<Canvas of={FormNavigationStories.Grouping} />

### Tilstander

Farger og ikoner baserer seg på tilstanden til hvert steg eller hver gruppe (`state = "idle" | "active" | "completed" | "invalid"`).

- `idle`: Standardtilstand for steg og grupper.
- `active`: Fylt sirkel - Indikerer det steget brukeren befinner seg på.
...
Kvitteringssiden bør være deaktivert inntil skjemaet er sendt inn.
...
For å skille stegene fra hverandre og unngå forvirring bør du unngå stegtitler som starter på samme måte, for eksempel "Program for ..." i flere steg.
