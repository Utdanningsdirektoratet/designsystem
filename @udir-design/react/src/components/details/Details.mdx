...
Hvis innholdet er for langt eller komplisert, bør du vurdere om teksten kan kortes ned eller omformuleres, om strukturen kan forbedres, om innholdet bør fordeles på flere sider, og om det kan være hensiktsmessig å benytte [`TableOfContents`](https://design.udir.no/beta/?path=/docs/components-tableofcontents--docs).
...
[`Chevron`-ikonet](https://aksel.nav.no/ikoner/ChevronDown) er bevisst plassert før teksten i leseretning, f.eks. på venstre side for norsk språk og høyre side for arabisk. Slik er det lettere for brukerne å se ikonet, spesielt om de har snevert synsfelt. Mange brukere tror også at de må peke på og velge ikonet for å åpne, og dette er enklere når ikonet kommer først.
