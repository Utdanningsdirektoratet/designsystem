...
Hvis innholdet er for langt eller komplisert, bør du heller omformulere teksten og/eller fordele den på flere sider.
...
Hold innholdet kort for å sikre at det lett kan relateres til overskriften.
Bryt innholdet opp i flere `Details` istedenfor å bruke flere overskrifter i én `Details`. Ved mye innhold kan du vurdere å lage en egen side i stedet. Dette gjør det enklere å finne tilbake til innholdet senere.
...
[`Chevron`](https://aksel.nav.no/ikoner/ChevronDown)-ikonet er bevisst plassert til venstre for teksten, av hensyn til brukere med nedsatt synsfelt. Der er det lettere for brukeren å se det (fordi vi leser fra venstre). Mange brukere tror at de må peke på og velge ikonet for å åpne.
...
<kbd>Tab</kbd> : Flytter fokus til neste element som kan ha fokus <br />
<kbd>Shift</kbd> + <kbd>Tab</kbd> : Flytter fokus til forrige element som kan ha
fokus <br />
<kbd>Space</kbd> : Aktiverer knapp <br />
<kbd>Enter</kbd> : Aktiverer knapp
