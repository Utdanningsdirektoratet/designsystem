...
Fra `useCheckboxGroup` får du `getCheckboxProps` som du bruker for å hente props som du sprer på checkboxene dine.
Du kan også hente ut `validationMessageProps`, `value` og `setValue`.

I `getCheckboxProps`-funksjonen er første parameter enten `value` — en unik identifikator for checkboxen — eller et objekt med props som kan legges på en `Checkbox`. Dersom du sender inn et objekt, må du passe på at det inneholder `value`.
...
<SimpleAlert type="info" heading={{ text: 'Tips', level: 3 }}>
