...
function isOdd(count: number): boolean {
  // This rejects all non-odd and non-integer numbers
  return count % 2 !== 1;
}
 ...
 * Returns hex color strings with perceptually uniform lightness steps.
 *
 * @param count - Number of colors to generate (odd number, minimum 3)
...
export function generateSequentialDivergentColors(
  ...
  if (count < 3 || isOdd(count)) {
    throw new RangeError(
      `count must be an odd number of at least 3, got ${count}`,
    );
  }

  // Equal-sized arms on each side of the neutral center
  const armCount = (count - 1) / 2;

  // Green arm key colors: darkest green → lightest green → neutral
  ...
  // Blue arm key colors: neutral → lightest blue → darkest blue
