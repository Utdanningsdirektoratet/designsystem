...
En React-hook for å styre navigasjon i [`FormNavigation`](/docs/components-formnavigation--docs).
...
```ts
{
  ...
  /** Props du kan spre på FormNavigation.Step for et gitt steg-ID eller props-objekt */
  getStepProps: (
  ...
  /** Props du kan spre på FormNavigation.Group for en gitt liste av steg-IDer */
...
Sett `label` i `getStepProps` for hvert steg dersom du ønsker å vise aktivt steg i en lukket gruppe. Da vil `label` på hvert steg overstyre eventuelle `children` i `FormNavigation.Step`.
...
Skjemavalidering håndteres utenfor hooken, slik at du kan tilpasse valideringslogikken etter behov. Når brukeren prøver å gå videre til neste steg, kan du validere gjeldende steg og bruke `markInvalid` eller `markCompleted` basert på resultatet. Dette kan gjøres i `onChange` som du sender til hooken.
...
Bruk `prev`, `next`, `hasPrev` og `hasNext` for å lage navigasjonsknapper i skjemaet.
